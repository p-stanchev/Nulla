...
- Optional relay/proxy (seed-only, use sparingly): `--relay-listen 0.0.0.0:28500 --relay-target <host:port>` forwards raw TCP between inbound connections and the target peer. Intended only for a few NAT’d miners; keep off otherwise. A capability-based automatic relay (no manual IPs) is planned with `--relay-auto`/`NULLA_RELAY_AUTO` + `--relay-cap`, and NAT’d nodes can request slots with `--request-relay`; see `docs/relay-automatic.md`.
