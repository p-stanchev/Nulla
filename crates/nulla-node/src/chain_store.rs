...
impl ChainDb {
    ...
    pub fn utxos_by_pubkey_hash(&self, pk_hash: [u8; 20]) -> Result<Vec<(OutPoint, UtxoRecord)>, String> {
        let mut out = Vec::new();
        for item in self.utxos.iter() {
            let (k, v) = item.map_err(|e| e.to_string())?;
            if k.len() != 36 {
...
impl ChainStore {
    ...
    pub fn utxos_for_pubkey_hash(&self, pk_hash: [u8; 20]) -> Result<Vec<(OutPoint, UtxoRecord)>, String> {
        self.db.utxos_by_pubkey_hash(pk_hash)
    }
